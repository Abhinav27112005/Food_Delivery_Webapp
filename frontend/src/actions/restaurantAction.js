import axios from "axios";
import {TOGGLE_VEG_ONLY,SORT_BY_RATINGS, ALL_RESTAURANT_FAIL, ALL_RESTAURANT_REQUEST, ALL_RESTAURANT_SUCCESS, SORT_BY_REVIEWS, CLEAR_ERROR } from "../constants/restaurantConstant"

export const getRestaurants=()=>{//we are going to take the data form outside source
    return async(dispatch)=>{
        try {
            dispatch({type:ALL_RESTAURANT_REQUEST});
        //code to fetch the data
        let link='/api/v1/eats/stores';

        const {data}= await axios.get(link);

        console.log(data);

        const {restaurants,count}=data;

        dispatch({
            type:ALL_RESTAURANT_SUCCESS,
            payload:{restaurants,count},
        });
        } catch (error) {
            dispatch({
                type:ALL_RESTAURANT_FAIL,
                payload:error.response.data.message,
            })
        }
    };
}

export const sortByRatings=()=>{
    return{
        type:SORT_BY_RATINGS,
    }
}
export const sortByReviews=()=>{
    return{
        type:SORT_BY_REVIEWS,
    }
}
export const toggleVegOnly=()=>{
    return{
        type:TOGGLE_VEG_ONLY,
    }
}
export const clearErrors=()=>{
    return{
        type:CLEAR_ERROR,
    }
}